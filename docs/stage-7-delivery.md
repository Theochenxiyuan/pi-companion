...
> 历史交付快照：本文记录截至 2026-07-24 的设置、协议、测试和开发安装状态，不代表当前 `main`。当前入口见 [`docs/README.md`](README.md)。
...
截至 2026-07-23，本交付还包含自定义 Provider 创建与编辑、模型启用范围、AI 任务元数据配置、任务历史交互收口、工作目录外附件安全暂存、共享排版与颜色 token、深色/浅色/跟随系统主题、WebView 界面缩放、Monitor 图标化展开/收起，以及 Explorer 菜单 Pi 图标。本文只作为阶段 7 完成时的交付事实快照；阶段 1–6 文档同样保留各阶段完成时的协议和能力。
