...
describe('desktop bridge protocol contract', () => {
  ...
  it('exposes persisted task templates as reusable composer drafts', () => {
    ...
    expect(promptComposerXaml).toContain('<Setter Property="MaxHeight" Value="64" />')
    expect(promptComposerXaml).toContain('Text="{Binding PromptPreview}"')
    expect(promptComposerXaml).toContain('TextWrapping="NoWrap"')
    ...
    expect(promptComposer).toContain('public string PromptPreview { get; } = string.Join(')
    expect(promptComposer).toContain('Prompt.Split((char[]?)null, StringSplitOptions.RemoveEmptyEntries)')
    expect(promptComposerXaml).toContain('x:Name="WorkspaceTrustConfirmationOverlay"')
    expect(promptComposerXaml).toContain('Content="不信任并开始"')
    expect(promptComposerXaml).toContain('Content="信任并开始"')
    expect(promptComposer).toContain('_piProjectTrust.GetStatus(draft.WorkingDirectory)')
    expect(promptComposer).toContain('_piProjectTrust.SetDecision(draft.WorkingDirectory, trusted)')
    expect(promptComposer).toContain('await StartDraftAsync(draft);')
  ...
  it('switches the focused task from an actual-position Monitor wheel picker', () => {
    ...
    expect(monitorXaml).toContain('StaysOpen="False"')
