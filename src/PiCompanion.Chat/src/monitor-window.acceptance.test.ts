...
describe('Monitor expand and collapse behavior', () => {
  ...
  it('shows an animated loading state while an enabled AI summary is pending', () => {
    expect(monitorCode).toContain('private bool _aiSummaryEnabled')
    expect(monitorCode).toContain('TaskSettings taskSettings')
    expect(monitorCode).toContain('taskSettings.AiSummaryEnabled')
    ...
    expect(monitorCode).toContain('var isAiSummaryLoading =')
    expect(monitorCode).toContain('var resultSummary = _aiSummaryEnabled')
    expect(monitorCode).toContain('? string.Empty')
    expect(monitorCode).toContain(': BuildLatestAgentMessageSummary(projection)')
