...
- Agent 可通过应用自带的 `create_task_template` 工具提出结构化模板并在用户单次确认后创建；即使任务使用完全访问也不得跳过确认。工具通过仅限当前 Windows 用户的私有双向 Named Pipe 调用应用服务，必须携带当前 Task、Run、Generation 和权限 Token，由应用校验后统一经过 `TaskTemplateRules` 与 Coordinator 保存，Extension 不直接读写 SQLite。
- Agent 创建的模板记录来源 Task 和 Run；重复的同内容请求返回已有模板。Agent 只拥有创建能力，不得通过该工具编辑、删除或直接运行模板。
...
- 定时任务仍是本阶段后续独立方向，不与任务模板或已冻结的 Skill 模块捆绑扩展。
...
阶段 8、阶段 9 的 Skill 与任务模板子阶段以及阶段 10 已有实现与验收事实；阶段 9 剩余的定时任务以及尚未启动的阶段 11，仍只记录正式发布前的功能方向。阶段分组和顺序可以在启动前调整，并以届时确认的产品目标、范围、交互、技术方案、风险和验收标准更新本文。
...
阶段 8 和阶段 10 已交付子阶段的完成事实分别以 `docs/stage-8-progress.md` 和 `docs/stage-10-progress.md` 为准。Skill 管理以本文第 8.8、16.6 和阶段 9 的完成边界为准；其验收必须覆盖原生发现、生效视图、内容指纹、本地导入、安全卸载、工作区信任和链接边界。任务模板以第 16.7 节和阶段 9 的完成边界为准。阶段 9 剩余的定时任务、阶段 11 及阶段 10 后续子阶段，在方案讨论完成后补充具体完成定义。第 5.3 节不属于当前 1.0 完成条件。
