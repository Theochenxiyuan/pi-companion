...
/// <summary>
/// Read-only discovery for Pi native skill locations. This service never
...
public sealed class SkillDiscoveryService
{
        ...
        IReadOnlyList<IgnoreRule> inherited)
    {
        ...
        foreach (var fileName in IgnoreFileNames)
        {
            ...
            {
                // Pi treats unreadable ignore files as best-effort.
            }
        }
        ...
        DiscoveryState state)
    {
        ...
        loser.Diagnostics.Add(new SkillDiagnostic(
            "name-collision",
            "warning",
            $"{context}中的 name \"{loser.Name}\" 冲突；Pi 保留优先级更高的 {winner.FilePath}。",
            loser.FilePath,
            winner.FilePath,
            workspace?.Id,
            workspace?.Name));
