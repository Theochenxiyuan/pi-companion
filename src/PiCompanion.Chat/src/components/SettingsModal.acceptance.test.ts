...
const snapshot: SettingsSnapshot = {
  values: {
    ...
    agent: { defaultModel: 'openai-codex/gpt-5.6-sol', defaultThinkingLevel: 'xhigh', autoCompact: true, autoRetry: true, cacheWarming: 'off', compactionReserveTokens: 16384, compactionKeepRecentTokens: 20000, retryMaxRetries: 3, retryBaseDelayMilliseconds: 2000, retryMaxDelayMilliseconds: 60000, steeringMode: 'one-at-a-time', followUpMode: 'one-at-a-time' },
...
describe('stage 7 settings modal', () => {
  ...
  it('creates a custom provider in the right detail pane and selects it after Pi saves it', async () => {
    ...
    expect(wrapper.get('button[aria-label="None 推理等级映射"]').text()).toBe('使用标准值（off）')
    expect(wrapper.find('input[aria-label="None 实际值"]').exists()).toBe(false)
    ...
    await wrapper.get('button[aria-label="Medium 推理等级映射"]').trigger('click')
    await wrapper.findAll('[role="option"]').find(option => option.text() === '使用标准值（medium）')!.trigger('click')
    ...
    expect(wrapper.get('button[aria-label="Max 推理等级映射"]').text()).toBe('使用标准值（max）')
