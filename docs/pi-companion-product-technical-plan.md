...
除已完成并在本文固化边界的 Skill 管理和任务模板外，本节其余条目只确认正式发布前的功能方向，不预设具体产品目标、交互、技术实现或验收标准；这些内容在进入对应阶段时再讨论并更新本文档。
...
- 产品只引入一个概念：任务模板是可复用的任务草稿，不是命令、Skill 或可执行工作流。
- 模板必填名称和任务内容；可选运行位置、模型、推理等级、权限偏好和“固定到新任务首页”。
- 运行位置可继承当前上下文、使用 Direct Chat、使用任意工作区或绑定一个现有工作区。
- 权限偏好只允许继承、只读或标准访问；模板不得预授权完全访问。
- 模板可用于新任务，也可填入兼容的现有任务输入区以发起后续一轮；套用后始终由用户检查并发送，不提供直接运行模板的 Bridge 命令。
- 新任务首页展示系统模板和用户固定模板；独立“任务模板”页负责完整列表与新建、编辑、复制、删除。
- 输入区 `+` 菜单只提供轻量模板选择器，用于快速套用，并可跳转到“任务模板”管理页。
- 系统模板不可编辑或删除，只能复制为用户模板；用户模板持久化到 SQLite。
- Agent 可通过应用自带的 `list_task_templates` 和 `get_task_template` 读取当前任务作用域可见的系统模板与用户模板；列表只返回轻量元数据，完整内容按 ID 单独读取，工作区绑定模板不得跨工作区暴露。
- Agent 可通过 `create_task_template` 提出结构化模板并在用户单次确认后创建；即使任务使用完全访问也不得跳过确认。Agent 创建的模板记录来源 Task 和 Run，重复的同内容请求返回已有模板；Agent 不得通过这些工具编辑、删除或直接运行模板。
- Agent Companion 工具通过仅限当前 Windows 用户的私有双向 Named Pipe 调用应用服务，必须携带当前 Task、Run、Generation 和权限 Token，由应用校验后统一经过规则层与 Coordinator 读写，Extension 不直接读写 SQLite。
- 模板不包含附件、Skill 选择、变量、斜杠命令、定时触发器、重试或通知策略。定时任务仍是独立产品方向，后续如复用模板，只消费模板解析后的任务草稿，不扩张模板本身的概念。
- 可选执行偏好只在目标任务允许修改时应用；空值继续继承当前设置。运行中的既有任务保持其固定设置，并在无法应用模板偏好时提示用户。

### 16.8 定时任务

- 定时任务是独立的持久化调度定义；每次触发创建新的 Task 和 Run，不续写旧 Session，也不切换 Chat 或 Monitor 当前选中的任务。
- 任务来源只有“自定义”和“关联模板”。“从模板填入”只是自定义编辑器中的复制动作，保存后不保留模板关系。
- 关联模板在每次触发时解析模板最新版本；模板必须绑定具体工作区或 Direct Chat。系统模板和继承当前上下文的模板只能用于快速填入，不能直接关联。
- 自定义定时任务保存自己的 Prompt、固定运行位置和可选模型、推理、权限偏好；工作区任务默认建议只读，定时定义不授予或预先批准运行中的交互权限。
- 第一版支持单次、每天、工作日和每周指定日期，按本地时间与显式时区计算。SQLite 保存 UTC `next_run_at`，休眠或应用未运行导致的 24 小时内错过会补跑一次，多次错过合并，超过窗口记为跳过。
- 同一计划已有活动 Run 时跳过新 occurrence，避免积压；其他并发继续服从全局 Run 槽和工作目录租约。
- 定时任务管理页提供启用/暂停、编辑、立即运行、打开最近创建的任务和删除。暂停或删除定义不取消已经创建的 Task，编辑只影响未来 occurrence。
- Agent 可通过 `list_scheduled_tasks` 读取当前工作区或 Direct Chat 可见的计划，并通过 `create_scheduled_task` 创建自定义或关联模板的计划。创建始终逐次确认且不能被完全访问跳过；AI 只能创建当前任务作用域内的计划，工作区自定义计划默认只读，重复定义返回已有计划。
- `scheduled_task_occurrences` 对计划与计划时间建立唯一约束，并保存触发时解析出的完整草稿，保证重启幂等与历史可追溯。
- 调度器运行在桌面进程后台并依赖应用驻留；完全退出时不会由 Windows 单独唤醒，恢复后按错过策略处理。
...
- 最近任务数量和副标题。
- AI 标题、AI 总结及其共享生成模型；生成语言统一跟随 Pi Companion 当前界面语言，两个开关只控制后续自动生成，不隐藏已经生成的元数据。
...
- 原“预置任务”方向已作为“任务模板”完成：模板是可复用任务草稿，支持系统模板、用户 CRUD、固定到首页和可选执行偏好；套用模板不会直接运行。
- 任务模板通过 SQLite migration 17 持久化，并通过 Bridge protocol 63 同步；Agent 可在用户逐次确认后创建带来源审计的模板，完整产品边界见第 16.7 节。
- 定时任务已作为独立调度模块交付：支持自定义任务、从模板快速填入、实时关联固定目标模板、单次/每天/工作日/每周触发、暂停、立即运行和 occurrence 审计；通过 SQLite migration 18 持久化并由 Bridge protocol 64 同步。实现与验收边界见 `docs/stage-9-scheduled-tasks-progress.md`。
...
阶段 8、阶段 9 和阶段 10 已有实现与验收事实；尚未启动的阶段 11 仍只记录正式发布前的功能方向。阶段分组和顺序可以在启动前调整，并以届时确认的产品目标、范围、交互、技术方案、风险和验收标准更新本文。
...
阶段 8、阶段 9 定时任务和阶段 10 已交付子阶段的完成事实分别以 `docs/stage-8-progress.md`、`docs/stage-9-scheduled-tasks-progress.md` 和 `docs/stage-10-progress.md` 为准。Skill 管理以本文第 8.8、16.6 和阶段 9 的完成边界为准；其验收必须覆盖原生发现、生效视图、内容指纹、本地导入、安全卸载、工作区信任和链接边界。任务模板以第 16.7 节和阶段 9 的完成边界为准，定时任务以第 16.8 节为准。阶段 11 及阶段 10 后续子阶段，在方案讨论完成后补充具体完成定义。第 5.3 节不属于当前 1.0 完成条件。
